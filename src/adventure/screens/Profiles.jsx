...
export function ParentGate({ onPass, onClose }) {
  ...
  return (
    <div className="pf-scrim" data-testid="profile-gate" onClick={onClose}>
      <div className="pf-card pf-gate" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="pf-text-btn" onClick={onClose}>Cancel</button>
      </div>
    </div>
  );
}
...
export function WelcomeScreen({ onStart, speak }) {
  ...
  return (
    <div className={`welcome welcome-${variant}`} data-screen-label="Welcome">
      {variant === 'calm' ? (
        <div className="wc-inner">
          ...
          <button className="wc-cta" data-testid="welcome-start" onClick={onStart}>Let’s go!</button>
        </div>
      ) : (
        <div className="wc-scene-wrap">
          ...
          <button className="wc-cta" data-testid="welcome-start" onClick={onStart}>Start the adventure</button>
        </div>
      )}
      {/* compare directions — temporary picker for you to choose */}
      <div className="wc-variant-pick" role="group" aria-label="Welcome style">
        <span>Style:</span>
        <button className={variant === 'calm' ? 'on' : ''} onClick={() => pick('calm')}>Calm</button>
        <button className={variant === 'scene' ? 'on' : ''} onClick={() => pick('scene')}>Scene</button>
      </div>
    </div>
  );
}
...
export function CreateProfile({ initial, onSave, onCancel, title = 'Make a profile' }) {
  ...
  return (
    <div className="pf-screen" data-screen-label="Create profile">
      <div className="pf-create">
        ...
        <label className="pf-field">
          ...
          <input data-testid="profile-name" type="text" value={name} maxLength={14}
        ...
        <div className="pf-buddy-grid">
          {MASCOT_CONCEPTS.map((c) => (
            <button key={c} className={`pf-buddy ${buddy === c ? 'on' : ''}`} data-testid={`profile-buddy-${c}`}
        ...
        <div className="pf-age-row">
          {AGES.map((a) => (
            <button key={a.k} className={`pf-age ${age === a.k ? 'on' : ''}`} data-testid={`profile-age-${a.k}`}
        ...
        <div className="pf-actions">
          {onCancel && <button className="wc-cta ghost" onClick={onCancel}>Back</button>}
          <button className="wc-cta" data-testid="profile-save" disabled={!ok}
...
export function ProfilePicker({ profiles, onPick, onAdd }) {
  return (
    <div className="pf-screen" data-screen-label="Who's playing">
      <h1 className="pf-h">Who’s playing?</h1>
      <div className="pf-pick-grid">
        {profiles.map((p) => (
          <button key={p.id} className="pf-pick-card" data-testid={`pick-${p.id}`} onClick={() => { advSfx('yes'); onPick(p.id); }}>
            <span className="pf-avatar"><Mascot concept={p.buddy} state="cheer" size={72} /></span>
            <b>{p.name}</b>
          </button>
        ))}
        <button className="pf-pick-card add" data-testid="pick-add" onClick={onAdd}>
...
export function ProfileSheet({ profiles, activeId, onSwitch, onAdd, onDelete, onClose }) {
  ...
  return (
    <div className="pf-scrim" data-testid="profile-sheet" onClick={onClose}>
      <div className="pf-card pf-sheet" onClick={(e) => e.stopPropagation()}>
        <div className="pf-sheet-head"><b>Profiles</b>
          <button className="gbtn white round" aria-label="Close" data-testid="profile-sheet-close" onClick={onClose} style={{ minHeight: 44, width: 44 }}>✕</button>
        </div>
        <div className="pf-sheet-list">
          {profiles.map((p) => (
            <div key={p.id} className={`pf-row ${p.id === activeId ? 'active' : ''}`}>
              <button className="pf-row-main" data-testid={`switch-${p.id}`} onClick={() => { if (p.id !== activeId) { advSfx('yes'); onSwitch(p.id); } }}>
              ...
              {profiles.length > 1 && <button className="pf-del" aria-label={`Remove ${p.name}`} data-testid={`del-${p.id}`} onClick={() => setConfirmDel(p)}>Remove</button>}
            </div>
          ))}
        </div>
        <button className="wc-cta" data-testid="profile-add" onClick={onAdd}>+ Add a profile</button>
        ...
        {confirmDel && (
          <div className="pf-confirm" data-testid="profile-del-confirm">
            ...
            <div className="pf-confirm-row">
              <button className="wc-cta ghost sm" onClick={() => setConfirmDel(null)}>Keep</button>
              <button className="wc-cta danger sm" data-testid="profile-del-yes" onClick={() => { onDelete(confirmDel.id); setConfirmDel(null); }}>Remove</button>
            </div>
          </div>
        )}
      </div>
