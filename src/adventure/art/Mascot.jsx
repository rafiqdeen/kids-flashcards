// Mascot.jsx — Pip & friends. Parametric SVG mascot.
// Ported from design_handoff_pip_cards_adventure/mascot.jsx to a real ES module
// (was attached to window.* in the prototype). BuddyContext flows the chosen
// hero buddy to every <HeroMascot>.
// <Mascot concept="pip|fox|owl|bear|bunny|monster" state="idle|cheer|encourage|point|sleep" size={120} flip decorative />
import { useContext } from 'react';
import { BuddyContext } from './buddy.js';
...
function Eyes({ c, state }) {
  const sleepy = state === 'sleep';
  const happy = state === 'cheer' || state === 'encourage';
  if (sleepy) {
    return (
...
function Arm({ c, side, pose }) {
  // side: 'l' | 'r'; pose: 'rest'|'up'|'point'|'wave'
  const flipX = side === 'l';
  let d;
...
export function Mascot({ concept = 'pip', state = 'idle', size = 120, flip = false, decorative = true }) {
  ...
  return (
    <svg viewBox="0 0 136 150" width={size} height={size * 150 / 136}
...
export function HeroMascot(props) {
  const buddy = useContext(BuddyContext);
  return <Mascot {...props} concept={buddy} />;
}
