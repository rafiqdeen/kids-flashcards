// Paint & Doodle studio. 1000×1000 internal canvas scaled to fit; pointer
// events with touch-action none; per-template WIP persistence; outline-aware
// flood fill; 480×480 export to the gallery.
...
// Outline templates — stroke-only guides on a 0..100 viewBox (mostly-closed
// paths so the fill bucket stays inside).
const OUTLINE = {
...
export function Paint({ onBack, onSaveArt, onRewards, mascot, speak }) {
  ...
  useEffect(() => {
    ...
    ctxRef.current = ctx;
    restore('cat');
    // mount-only canvas bootstrap
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const storeKey = (t) => `pip-doodle-${t}`;
  const persist = () => { try { localStorage.setItem(storeKey(template), canvasRef.current.toDataURL('image/png')); } catch { /* quota */ } };
  const clearCanvas = () => ctxRef.current.clearRect(0, 0, RES, RES);
  const restore = (t) => {
    clearCanvas();
    let data; try { data = localStorage.getItem(storeKey(t)); } catch { /* private mode */ }
  ...
  const pushUndo = () => {
    try {
      ...
      setCanUndo(true);
    } catch { /* tainted/oom — undo unavailable */ }
  };
  ...
  const dab = (p) => {
    ...
    if (brushType === 'sparkle') {
      ...
      drawStar(ctx, p.x + (Math.random() - 0.5) * brush, p.y + (Math.random() - 0.5) * brush, brush * 0.25, Math.random() * Math.PI);
      return;
    }
  ...
  // ---- magic fill (flood fill bounded by outline + existing paint) ----
  const floodFill = (sx, sy) => {
    ...
    if (template !== 'blank' && OUTLINE[template]) {
      ...
      try { bctx.stroke(new Path2D(OUTLINE[template])); } catch { /* Path2D unsupported */ }
      bctx.restore();
    }
    bctx.drawImage(canvasRef.current, 0, 0);
    const bd = bctx.getImageData(0, 0, RES, RES).data;
    const seedIdx = sy * RES + sx;
    if (bd[seedIdx * 4 + 3] > 40) return false; // tapped a line or paint
    pushUndo();
    ...
    // composite the fill UNDER the painting so brush lines stay crisp on top
  ...
  const start = (e) => {
    e.preventDefault();
    try { canvasRef.current.setPointerCapture(e.pointerId); } catch { /* synthetic/stale pointer */ }
    const p = pos(e);
    if (tool === 'fill') {
      if (floodFill(p.x, p.y)) { setHasInk(true); persist(); speak('Whoosh!'); announce('Whoosh!'); }
      return;
    }
    pushUndo();
  ...
  const exportArt = () => {
    ...
    if (template !== 'blank' && OUTLINE[template]) {
      ...
      try { ctx.stroke(new Path2D(OUTLINE[template])); } catch { /* Path2D unsupported */ }
      ctx.restore();
    }
  ...
  const freshPage = () => {
    ...
    try { localStorage.removeItem(storeKey(template)); } catch { /* private mode */ }
  };
